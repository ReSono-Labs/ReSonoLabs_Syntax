...
static void expire_session_if_needed(void)
{
    ...
    if (now_ms() - s_last_activity_ms <= SESSION_TIMEOUT_MS) {
        return;
    }
...
bool security_auth_init(void)
{
    ...
    if (storage_platform_get_str(SECURITY_NAMESPACE, KEY_DEV_PIN, pin, sizeof(pin))) {
        return true;
    }
    return security_auth_rotate_pin(pin, sizeof(pin));
}

bool security_auth_get_pin(char *buf, size_t buf_len)
{
    if (buf == 0 || buf_len < 7 || !security_auth_init()) {
        return false;
    }
